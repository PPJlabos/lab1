...
#    #printlen(inputs)  ## debug print
    for line in range (len(inputs)):
        ...
        # spremi definicije regex izraza u dict regex
                ...
                regex_name = inputs[line].split(' ')[0]
                print regex_name
                prioritetna_lista_regexa.append(regex_name)
                regex[regex_name] = inputs[line].split(' ')[1]


    # dopuni regularne izraze (zamjeni skracenice s punim regularnim izrazom)
        print prioritetna_lista_regexa
        print regex
   
       
        for reg in prioritetna_lista_regexa:
            for key in prioritetna_lista_regexa:    
                if key in regex[reg]:# or reg in regex[key]:
                    regex[reg] = regex[reg].replace(key, "(" + regex[key] + ")")
                    #regex[key] = regex[key].replace(key, "(" + regex[reg] + ")")
    print regex
